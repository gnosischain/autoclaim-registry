...
contract ClaimActionUpgradeable is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function execute(
        ...
        // Step 1: Swap on Balancer
        _balancerSwap(tokenIn, tokenOut, amountIn, minOut);

        // Step 2: Forward funds to recipient
        uint256 balance = IERC20(tokenOut).balanceOf(address(this));
        IERC20(tokenOut).transfer(recipient, balance);
    }

    function _balancerSwap(address tokenIn, address tokenOut, uint256 amountIn, uint256 minOut) internal {
        // Swap logic here using Balancer or Curve
    }
	...
	function swapAndForward(address claimAddress, uint256 amount) public {
		...
		IERC20(gnoTokenAddress).transferFrom(claimAddress, address(this), amount);
